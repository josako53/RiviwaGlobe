...
from schemas.lifecycle import LogAction
...
async def log_action(feedback_id: uuid.UUID, body: LogAction, db: DbDep, kafka: KafkaDep, token: StaffDep) -> dict:
    return action_out(await _svc(db, kafka).log_action(feedback_id, body.model_dump(exclude_none=True), by=token.sub))
